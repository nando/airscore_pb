<?php
define('BINDIR', '/var/www/airscore/cgibin/');
define('FILEDIR', '/var/www/airscore/tracks/');
...
function db_connect()
{
    $link = mysql_connect('localhost', 'xc', 'gogo')
...
function waypoint_select($link,$tasPk,$name,$selected)
{
    ...
    while($row = mysql_fetch_array($result))
    {
        $rwpPk = $row['rwpPk'];
        $rname = $row['rwpName'];
        $rdesc = $row['rwpDescription'];
        $waypoints["$rname $rdesc"] = $rwpPk;
    }
    //ksort($waypoints);
