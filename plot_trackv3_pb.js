...
jQuery(window).load(function(){
  jQuery("#trackid").bind("change", function(){
    do_add_track(0);
  });
  jQuery("#toggle_fullscreen").bind("click", function(e){
    e.preventDefault();
    var nofull = jQuery('.no-fullscreen');
    var link = $(this);
    var img = $('img', link);
    if(nofull.length>0) {
      nofull.removeClass('no-fullscreen');
      nofull.addClass('fullscreen');
      link.attr('title','Vista normal');
      img.attr('src','/airscore/images/no-fullscreen.png');
    }
    else {
      var full = jQuery('.fullscreen');
      full.removeClass('fullscreen');
      full.addClass('no-fullscreen');
      link.attr('title','Ver en pantalla completa');
      img.attr('src','/airscore/images/fullscreen.png');
    }
    //alert(google);
...
function do_add_track(x) 
{
    ...
    if (!x || x == 0)
    {
      var track_select = jQuery("#trackid");
      trackid = track_select.val();
      jQuery('option:selected', track_select).remove();
    }
    else
    {
        trackid = x;
...
function plot_track(jstr)
{
    ...
    segments = Array();
    trklog = Array();
    color = (number_of_tracks % 7)+1;
    maxalt = 0;
    for (row in track) {
      if(track[row][3]>maxalt)
        maxalt = track[row][3];
    }
    for (row in track)
    {
...
function plot_track_header(body)
{
    ...
    var color = (number_of_tracks % 7)+1;
    var border_color = sprintf("#%02x%02x%02x",210*((color&0x4)>>2) ,210*((color&0x2)>>1),210*(color&0x1)); 
    var back_color = sprintf("#%02x%02x%02x",(255*((color&0x4)>>2)|212) ,(255*((color&0x2)>>1)|212),(255*(color&0x1)|212)); 
    positions[body["initials"]] = number_of_tracks-1;
    ihtml = "<div style=\"border-color:"+border_color+"; background-color:"+back_color+";\" class=\"trackInfo\"><b>" + body["name"] + "</b><br>\n";
...
function plot_glider(glider,lat,lon,alt)
{
    ...
    // load image ...
    html = "<center>" + onscreen[glider]["initials"] + "<br><img src=\"pger" + 
       (positions[onscreen[glider]["initials"]]) + ".png\"></img><br>"+Math.floor(alt/10)+"</center>";
    if (alt > 999)
    {
...
function reset_map()
{
    ...
    animate_init();
    document.getElementById("play_or_pause").setAttribute('src','/airscore/images/play.png');
    current == -1;
}
function pause_map()
{
    if (pause == 0)
    {
        clearTimeout(timer);
        pause = 1;
        document.getElementById("pause").setAttribute('title','Iniciar animación');
        document.getElementById("play_or_pause").setAttribute('src','/airscore/images/play.png');
    }
    else
    {
        ...
        pause = 0;
        document.getElementById("pause").setAttribute('title','Parar animación');
        document.getElementById("play_or_pause").setAttribute('src','/airscore/images/pause.png');
    }
}
function forward()
{
